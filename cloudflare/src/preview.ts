...
*{box-sizing:border-box}body{margin:0;background:#f4f5f7;color:#222;font-family:Arial,"Noto Sans KR",sans-serif;line-height:1.8}.wrap{max-width:760px;margin:auto;padding:28px 16px 60px}.preview-bar{background:#111827;color:#fff;border-radius:12px;padding:12px 16px;margin-bottom:18px;font-size:13px}.post{background:#fff;border-radius:18px;padding:28px 24px;box-shadow:0 3px 18px rgba(0,0,0,.06)}.section-label{font-size:12px;font-weight:800;letter-spacing:.04em;color:#6b7280;margin:0 0 7px}h1{font-size:29px;line-height:1.4;margin:0 0 20px}h2{font-size:21px;margin:28px 0 10px;border-left:4px solid #111827;padding-left:10px}p{margin:0 0 17px}.greeting{font-size:16px}.disclosure{background:#f8fafc;border-radius:10px;padding:12px 14px;color:#5b6470;font-size:12px;margin-bottom:22px}.product{padding:14px;background:#f8fafc;border-radius:12px;margin:20px 0;font-size:13px}figure{margin:24px 0;text-align:center}figure img{display:block;width:100%;max-height:460px;object-fit:contain;border-radius:12px;border:1px solid #e5e7eb;background:#fff}figcaption{font-size:11px;color:#8a919b;margin-top:5px}.partner{display:inline-block;margin-top:22px;padding:12px 18px;border-radius:10px;background:#111827;color:#fff;text-decoration:none;font-weight:700}.fact{margin-top:20px;padding:12px 14px;border:1px solid #e5e7eb;border-radius:10px;font-size:12px;color:#666}.back{display:inline-block;margin-bottom:12px;color:#374151;text-decoration:none;font-size:13px}@media(max-width:600px){.wrap{padding:16px 10px 40px}.post{padding:22px 17px}h1{font-size:24px}h2{font-size:19px}}
</style></head><body><main class="wrap"><div class="preview-bar">📱 게시 전 미리보기 · 실제 블로그에 올리기 전 최종 확인용</div><article class="post"><div class="section-label">게시 제목</div><h1>${escapeHtml(title)}</h1><div class="disclosure">${escapeHtml(blog.disclosure ?? "")}</div><div class="product"><strong>선정 상품</strong><br>${escapeHtml(product.productName ?? "")}</div><div class="section-label">본문</div><div class="body">${renderBody(blog.body ?? "", images)}</div>${partnerUrl ? `<a class="partner" href="${escapeHtml(partnerUrl)}" target="_blank" rel="noopener noreferrer">상품 확인하기</a>` : ""}<div class="fact">상품 정보와 배송 조건은 생성 당시 쿠팡 API 검색 결과를 기준으로 작성되었습니다. 실제 판매 페이지의 최신 정보와 차이가 있을 수 있습니다.</div></article></main></body></html>`;
