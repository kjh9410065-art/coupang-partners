...
export function buildProductPost(keyword: string, productName: string, researchEvidence: string[] = []) {
  // 조사 결과가 상품명 하나뿐인 경우에는 특징 근거로 사용하지 않습니다.
  ...
  const intro = hasResearch
    ? `공개 조사에서 확인된 제품 정보를 중심으로 살펴보겠습니다. ${research}`
  ...
  const reason = hasResearch
    ...
    : "현재 확인된 자료만으로 특정 기능이나 성능을 근거로 추천 이유를 만들지 않았습니다.";

  const advantages = hasResearch
    ? `공개 자료에서 확인되는 제품 정보를 기준으로 필요한 용도와 비교하기 좋습니다. ${research.slice(0, 400)}`
  ...
  const disadvantages = hasResearch
    ...
    : "현재 공개 자료가 부족해 특정 부분을 단점이라고 사실처럼 적지 않았습니다.";

  const goodPlace = hasResearch
    ? "공개 자료에서 확인된 용도와 실제 사용 목적이 맞는 공간에서 활용하기 좋습니다. 집이나 작업 공간 등 실제 사용할 장소와 제품 정보를 함께 비교해보세요."
